...
# =============================================================================
#  zd-transfer — Truly Automated Setup for Linux / Ubuntu
#  =============================================================================
#  This script installs everything needed to run the Zendesk Configuration
#  Transfer tool, including Docker, Python dependencies, and the Docker
#  Compose stack. Run it once after cloning the repo.
# =============================================================================
...
# Evaluate Docker execution permissions for the current session
DOCKER_CMD="docker"
...
# ------------------------------------------------------------------
#  6. Generate Docker secrets (Bulletproof)
# ------------------------------------------------------------------
log_info "Ensuring cryptographic secrets exist in docker/.env..."

mkdir -p docker
# Create base docker/.env if it doesn't exist
...
# Helper function to reliably inject secrets
ensure_secret() {
    local key="$1"
    local generate_cmd="$2"
    local file="docker/.env"
    
    # Extract current value (ignoring errors if grep finds nothing)
    local current_val
    ...
    if [ -z "$current_val" ] || [ "$current_val" = "change_me" ]; then
        ...
        # Strip old key (if it exists) and append the new one
...
# Inject or generate required variables
